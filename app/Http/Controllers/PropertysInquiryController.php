    ...
    public function getPropertyInquiryList()
    {
        ...
        foreach ($res as $row) {
            ...
            if ($row->property->added_by == 0) {
                $operate1 = '<a  id="' . $row->id . '"  class="btn icon btn-primary chatdata btn-sm rounded-pill"  data-bs-toggle="modal" data-bs-target="#chat_modal"  onclick="setallMessage(' . $row->propertys_id . ',' . $row->customers_id . ');"" title="Chat"><i class="fas fa-comment"></i></a>';
            } else {
                $operate1 = '';
            }

            $operate .=  '<a class="btn icon btn-info btn-sm rounded-pill view-property"  data-bs-toggle="modal" data-bs-target="#ViewPropertyModal" title="View Details"><i class="fas fa-car"></i></a>&nbsp;&nbsp;';
            $tempRow['id'] = $row->id;
            ...
            if ($row->status == '0') {
                $tempRow['status'] = '<span class="badge bg-primary">'. __('Pending') .'</span>';
            }
            if ($row->status == '1') {
            ...
            $tempRow['category'] = $row->property->category->category_ar;
            $tempRow['manufacturer'] = $row->property->manufacturer->manufacturer_ar;
            $tempRow['model'] = $row->property->model->model;
            $tempRow['year'] = $row->property->year->year;
            $tempRow['city'] = $row->property->city->city_ar;
            $tempRow['area'] = $row->property->area->area_ar;
